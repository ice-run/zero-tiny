...
# 设置密码函数
set_password() {
  # 优先从 ${APP_DIR}/mysql/${ENV_FILE} 或 docker/${ENV_FILE} 读取已存在的 MYSQL_PASSWORD
  if [ -z "$MYSQL_PASSWORD" ]; then
    if [ -f "${APP_DIR}/mysql/${ENV_FILE}" ]; then
      MYSQL_PASSWORD=$(grep -E '^MYSQL_ROOT_PASSWORD=' "${APP_DIR}/mysql/${ENV_FILE}" | cut -d'=' -f2-)
  ...
  # 优先从 ${APP_DIR}/redis/${ENV_FILE} 或 docker/${ENV_FILE} 读取已存在的 REDIS_PASSWORD
  if [ -z "$REDIS_PASSWORD" ]; then
    if [ -f "${APP_DIR}/redis/${ENV_FILE}" ]; then
      REDIS_PASSWORD=$(grep -E '^REDIS_PASSWORD=' "${APP_DIR}/redis/${ENV_FILE}" | cut -d'=' -f2-)
    elif [ -f "docker/${ENV_FILE}" ]; then
...
create_dir() {
  ...
  if [ ! -f "${APP_DIR}/${ENV_FILE}" ]; then
    log_info "复制 ${ENV_FILE} 文件到 ${APP_DIR} ..."
    cp "${WORK_DIR}/docker/${ENV_FILE}" "${APP_DIR}/" || log_error "复制 ${ENV_FILE} 文件失败！"
    chmod 644 "${APP_DIR}/${ENV_FILE}" || log_warn "设置 ${ENV_FILE} 文件权限失败！"
    log_debug "${ENV_FILE} 文件复制成功"
  else
    log_debug "${ENV_FILE} 文件已存在: ${APP_DIR}/${ENV_FILE}"
  fi
...
# 替换环境变量
replace_envs() {
  # 将目录中的 ${ENV_FILE} 文件中的变量替换为实际值 APP_DIR=${APP_DIR} DATA_DIR=${DATA_DIR} LOGS_DIR=${LOGS_DIR}
  log_debug "替换 ${ENV_FILE} 文件中的变量..."
  sed -i "s|APP_DIR=.*|APP_DIR=${APP_DIR}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 APP_DIR 变量失败！"
  sed -i "s|DATA_DIR=.*|DATA_DIR=${DATA_DIR}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 DATA_DIR 变量失败！"
  sed -i "s|LOGS_DIR=.*|LOGS_DIR=${LOGS_DIR}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 LOGS_DIR 变量失败！"
  sed -i "s|CODE_DIR=.*|CODE_DIR=${WORK_DIR}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 CODE_DIR 变量失败！"
  sed -i "s|DOCKER_REGISTRY=.*|DOCKER_REGISTRY=${DOCKER_REGISTRY}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 DOCKER_REGISTRY 变量失败！"
  sed -i "s|REGISTRY_NAMESPACE=.*|REGISTRY_NAMESPACE=${REGISTRY_NAMESPACE}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 REGISTRY_NAMESPACE 变量失败！"
  sed -i "s|IMAGE_TAG=.*|IMAGE_TAG=${IMAGE_TAG}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 IMAGE_TAG 变量失败！"
  sed -i "s|MYSQL_PASSWORD=.*|MYSQL_PASSWORD=${MYSQL_PASSWORD}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 MYSQL_PASSWORD 变量失败！"
  sed -i "s|REDIS_PASSWORD=.*|REDIS_PASSWORD=${REDIS_PASSWORD}|" "${APP_DIR}/${ENV_FILE}" || log_error "替换 REDIS_PASSWORD 变量失败！"
...
pull_image() {
  ...
  # 如果 REGISTRY_NAMESPACE = "ice-run-open"，则不需要登录
  if [ "${REGISTRY_NAMESPACE}" = "ice-run-open" ]; then
  ...
  docker pull "${DOCKER_REGISTRY}/${REGISTRY_NAMESPACE}/${image_name}:${IMAGE_TAG}" || log_error "拉取 Docker 镜像 ${image_name} 失败！"
  log_debug "拉取 Docker 镜像 ${image_name} 成功！"
  # docker tag "${DOCKER_REGISTRY}/${REGISTRY_NAMESPACE}/${image_name}:${IMAGE_TAG}" "${image_name}" || log_error "标记 Docker 镜像 ${image_name} 失败！"
